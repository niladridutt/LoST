...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_rectangular_sincos_pos_embed(self.pos_embed.shape[-1], h, w)
...
def get_rectangular_sincos_pos_embed(embed_dim, grid_size_h, grid_size_w):
    """
    Create sinusoidal positional embeddings for a rectangular grid.
    """
    grid_h = np.arange(grid_size_h, dtype=np.float32)
    grid_w = np.arange(grid_size_w, dtype=np.float32)
    grid = np.meshgrid(grid_w, grid_h)  # grid_w corresponds to columns, grid_h to rows
    grid = np.stack(grid, axis=0)
